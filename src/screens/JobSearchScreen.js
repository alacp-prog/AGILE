...
export default function JobSearchScreen() {
  ...
  const submitApplication = async () => {
    if (!resumeFile || !coverLetterFile) {
      alert("Veuillez d'abord sélectionner un CV et une lettre de motivation.");
      return;
    }
    ...
    try {
      // 1. Save files locally using expo-file-system
      const appDir = FileSystem.documentDirectory + 'applications/';
      ...
      const newResumeName = `cv_${Date.now()}.${resumeExt}`;
      const localResumeUri = appDir + newResumeName;
      
      const coverExt = coverLetterFile.name.split('.').pop() || 'pdf';
      const newCoverName = `lm_${Date.now()}.${coverExt}`;
      const localCoverUri = appDir + newCoverName;
      ...
      await FileSystem.copyAsync({
        from: coverLetterFile.uri,
        to: localCoverUri
      });
      
      // 2. Save application record to Firestore
      const applicationData = {
        ...
        resumeLocalUri: localResumeUri,
        resumeName: resumeFile.name,
        coverLetterLocalUri: localCoverUri,
      ...
      alert("Une erreur s'est produite lors de l'envoi de la candidature.");
            ...
            <View className="bg-white rounded-t-3xl px-6 pt-5 pb-8 min-h-[60%]">
              ...
              {applicationStatus === 'form' && (
                <ScrollView showsVerticalScrollIndicator={false} className="mb-4">
                  ...
                  <View className="mb-5">
                     <Text className="text-sm font-bold text-slate-700 mb-2">Lettre de motivation *</Text>
                     <TouchableOpacity 
                      className={`border-2 ${coverLetterFile ? 'border-solid border-indigo-500 bg-indigo-50' : 'border-dashed border-slate-300 bg-slate-50'} rounded-xl p-4 flex-row items-center justify-center`}
                      onPress={() => handleDocumentPick('coverLetter')}
                    >
                      <Ionicons name={coverLetterFile ? "document" : "document-text-outline"} size={24} color={coverLetterFile ? "#4f46e5" : "#94a3b8"} />
                      <Text className={`ml-2 font-medium ${coverLetterFile ? 'text-indigo-700' : 'text-slate-500'}`}>
                        {coverLetterFile ? coverLetterFile.name : 'Uploader un document (PDF, DOCX)'}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </ScrollView>
